...
def plot_adc(file = None, nodeId = 0, bus_id = 1, xlimit = 200, ylimit = 1):
    ...
    for target in np.arange(_start_a,n_channels+_start_a):
        ...
        plt.suptitle('MOPS (NodeId = %i & bus = %i]'%(nodeId,bus_id) ,fontsize=8)
        plt.errorbar(respondant[respondant["nodeId" ]== nodeId]["time"],
                     respondant[respondant["nodeId" ]== nodeId]["adc_data_converted"],
                     markeredgewidth=1,
                     fmt='.k',
                     elinewidth=2)
        ax.plot(respondant[respondant["nodeId" ]== nodeId]["time"],respondant[respondant["nodeId" ]== nodeId]["adc_data_converted"], 
                label="ADC channel No.%i"%target, color =get_color(target))
        ...
        print("Plotting data from channel %i [Max time = %d s]"%(target,ax.get_xlim()[1]))
        if ylimit:
...
def plot_can_compare_bar(file = "Configuration_pattern.csv",test = "test_rx"):
    ...
    respmsg_append=[]
    status_array = []
    data_file = pd.read_csv(file,encoding = 'utf-8', dtype='unicode').fillna(0)
    ...
    for i in n_buses["bus_id"]:    
        ...
        req_msg_cond    = np.where((data_file["bus_id"]==str(i)) & (data_file[test]==str(1)) & (data_file["reqmsg"]==str(1)))
        resp_msg_cond   = np.where((data_file["bus_id"]==str(i)) & (data_file[test]==str(1)) & (data_file["respmsg"]==str(1)))
        status_cond     = np.where((data_file["bus_id"]==i) & (data_file[test]==str(1)))
        ...
        cond_bus_status  = (data_file.loc[status_cond  ]["status" ].value_counts())            
        #append the results
    ...
    print("In Bus: %s\n Request Messages:%d\n Response Messages:%d\n Efficiency:%d\n Passed msg:%d\n Failed msg:%d\n"%(i, reqmsg_append[0],respmsg_append[0], eff,
                                                                                       data_file[data_file["status"]==str(1)].shape[0],
                                                                                       data_file[data_file["status"]==str(0)].shape[0]))
    ax.grid()
...
def plt_mopshub_grid(file = "file"):
    fig = plt.figure()
    ax = fig.add_subplot(111)
...
file1="../output_data/mopshub_data_0_1_0.csv"
file2="../output_data/mopshub_top_32bus.csv"
#plot_can_compare_bar(file=os.path.expanduser('~')+"/tb_mopshub_top_tb_mopshub_top_32bus.data_generator0.can_bus_activity0_buses_1.csv",test = "test_tx")
#plot_can_compare_bar(file=os.path.expanduser('~')+"/tb_mopshub_top_tb_mopshub_top_16bus.data_generator0.can_bus_activity0_buses_1.csv",test = "test_tx")
#plt_mopshub_grid(file = file)
plot_can_compare_bar(file=file2,test = "test_tx")
plot_adc(file =file2, nodeId = 0, xlimit = 20000,ylimit = False)

#plot_can_compare_bar(file=file1,test = "test_tx")
#plot_adc(file =file1, nodeId = 0, range = [0,200])
PdfPages.close()
