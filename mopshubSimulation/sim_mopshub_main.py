...
from mopshubSimulation import sim_mops_main 
import struct
# Third party modules
...
class SIMMOPSHUB(object):#READSocketcan):#Instead of object
    ...
    def __init__(self,
        ...
        super(SIMMOPSHUB, self).__init__()  # super keyword to call its methods from a subclass:        
        ...
        if logfile is not None:
            ts = os.path.join(logdir, time.strftime('%Y-%m-%d_%H-%M-%S_sim_Wrapper.'))
            self.logger.info(f'Existing logging Handler: {ts}'+'log')
            self.logger_file = Logger().setup_file_logger(name = "simWrapper",console_loglevel=console_loglevel, logger_file = ts)#for later usage
            self.logger_file.success('....Done Initialization!')
    
    def set_hw_connection(self):
        self.logger.info(f'Starting a Thread for MOPSHUB')
        #self.__simMsgThread = Thread(target=self.read_sim_mopshub_message, args=(["reg0","reg1","reg2"], 0.001,0x0, None))#self.between_thread_callback)
        #self.__simMsgThread.start()  
                               

    def call_back_read_sim_mopshub_message(self):
        self.read_sim_mopshub_message(reg = ["reg0","reg1","reg2"], timeout=0.001, out_msg = None)
        
    def config_sim_hardware(self, uri = None, addressFilePath = None):
        
        if uri is None:
            uri = self.__uri
        if addressFilePath is None: 
            addressFilePath = self.__addressFilePath 
        self.logger.info(f'Getting connected device under%s [file:%s]'%(uri,addressFilePath))
        self.set_sim_hardware("sim")
    ...
    def read_sim_message(self, hw = None, registerName=None, timeout=None, out_msg = True, w_r= "Read"):
        """Read incoming sim messages without storing any Queue
        node: obj: sim._core type 
        registerName: str: 
        """
        reg_value = 1
    ...
    def write_sim_mopshub_message(self, hw =None, data=None,reg =None, timeout=None, out_msg= True):
        ...
        nodes = []
        reqmsg = 0
        [self.write_sim_message(hw =hw, data=d, registerName=r, timeout=timeout, out_msg=out_msg) for r,d in zip(reg,data)]
        reqmsg = 1
        return reqmsg

    def write_sim_message(self,hw =None, data=None, registerName=None, timeout=None, out_msg = True ):
        reqmsg= 1
        reg_ret_value = self.read_sim_message(hw = hw, registerName=registerName, timeout=timeout, out_msg = out_msg, w_r= "Writing") 
        status = (hex(data) ==hex(reg_ret_value)) 
        if out_msg and not status:  self.logger.warning(f'Writing mismatch in {registerName} [W: {hex(data)} |R:{hex(reg_ret_value)}]') 
        return reqmsg         
    ...
    def dumpMessage(self, cobid, msg, t):
        """Dumps a sim message to the screen and log file
        Parameters
        ----------
        cobid : :obj:`int`
            |CAN| identifier
    ...
    def enable_read_signal (self,timeout =None, out_msg = None):
        ...
        hw = self.get_sim_hardware()
        count, count_limit = 0, 2
        #irq_tra_sig = 0x0
        while (count !=count_limit):
            irq_tra_sig =   self.read_sim_message(hw = hw, registerName="reg3", timeout=timeout, out_msg = None)
            count= count+1
    ...
    def read_sim_mopshub_message(self, reg =None, timeout=None,  nodeId = 0x0, out_msg = True):
        hw = self.get_sim_hardware()
        nodes = []
        reg_values = []
        respmsg = 0
        ...
        if msg_found:
            sdo_cobid =0x580+nodeId
            reg_values = np.append(reg_values,hex(sdo_cobid))
            #reg_values = np.append(reg_values,0x43)    
            
            for r in reg[1:]: 
                reg_value = randint(1,50)
                # Send IPbus transactions
                if out_msg: self.logger.info(f'Read {r} Value = {hex(reg_value)}')
                reg_values = np.append(reg_values,hex(reg_value))    
            respmsg = 1
            t = time.time()
            ...
            self.__simMsgQueue.appendleft((cobid, data_ret, t))
    ...
    def  return_valid_message(self, nodeId, index, subindex, cobid_ret, data_ret, SDO_TX, SDO_RX,timeout):
        ...
        while time.perf_counter() - t0 < 1 and messageValid_queue == False:
                ...
                for i, (cobid_ret, data_ret, t) in zip(range(len(self.__simMsgQueue)), self.__simMsgQueue):
                    ...
                    if (messageValid_queue):
                        del self.__simMsgQueue[i]
                        break
                if (messageValid_queue ==False):
                    _, _, respmsg, responsereg, _ = self.read_sim_mopshub_message(reg = ["reg0","reg1","reg2"], nodeId = nodeId, timeout=timeout, out_msg = None) 
                #self.__simMsgThread.join()
    ...
    def read_adc(self,hw, bus_id,timeout):
        ...
        for address in self.__address_byte:
            ...
            _, _, _, adc_out =  self.read_monitoring_sim(hw =hw,
                                                              cobid = 0x20,
                                                              spi_reg =address,
                                                              spi_select=bus_id,
                                                              timeout=timeout, 
    ...
    def read_monitoring_sim(self,hw =None, spi_reg =None, spi_select=None,cobid = 0X20, timeout=1,out_msg=None):
        status =0;
        respmsg, responsereg  = 0, None
        ...
        mon_reg6_hex = Analysis().binToHexa(bin(cobid)[2:].zfill(8)+
                                            ...
                                            bin(0)[2:].zfill(8)) 
        reqmsg =  self.write_sim_mopshub_message(hw =hw, 
                                              data=[mon_reg6_hex,0x0,0x0,0xa], 
                                              reg = ["reg6","reg7","reg8","reg9"], 
                                              timeout=timeout, 
                                              out_msg = out_msg)     
        #read the response from the socket
        _frame  =  self.read_sim_mopshub_message(reg = ["reg0","reg1","reg2"], 
                                                        timeout=timeout, 
                                                        nodeId = nodeId,
                                                        out_msg = out_msg) 
     #   hw.dispatch()
    ...
    def read_sdo_sim(self,hw= None, bus= None, nodeId=None, index=None, subindex=None, timeout=1, SDO_TX=0x600, SDO_RX=0x580,out_msg=None):
            ...
            for bus in bus_range:
                for node in nodeId:    
                    ...
                    for c in tqdm(np.arange(len(_channelItems)),colour="green"):
                        ...
                        subindex = channel - 2
                        data_point, reqmsg, requestreg, respmsg,responsereg , status =  self.read_sdo_sim(hw =hw,
                                                                                                           bus = bus, 
                                                                                                           nodeId= node, 
